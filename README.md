...
terraform/            VM, LXC resolver, Talos bootstrap, ArgoCD install. Nothing else.
  ...
  secrets.tf          the ONE Secret Terraform writes: ESO's Key Vault login, read from the vault
...
cluster/lab/secrets/    ClusterSecretStore (Azure Key Vault) + one ExternalSecret per credential (wave -6)
...
scripts/              keyvault.ps1 (fills the vault), preflight.ps1 (before apply), verify.ps1 (after), credentials.ps1 (every password)
```
...
```powershell
az login
./scripts/keyvault.ps1             # creates the Key Vault and every credential in it
...
3. Cloudflare API token (Zone/DNS/Edit + Zone/Zone/Read on `ryfoje.com`),
   ...
   `tls:` block.
4. An Azure subscription: `az login`, then `scripts/keyvault.ps1` creates the
   vault and fills it. That one run is the whole of Azure.
5. This repo, public, at `git_repo_url`.

## Secrets

No credential is in git and none is generated by Terraform. Every one lives
in Azure Key Vault and reaches the cluster through External Secrets Operator:
`cluster/lab/secrets/` has the `ClusterSecretStore` and one `ExternalSecret`
per Secret, `scripts/keyvault.ps1` holds the same list as the catalog it
writes, and DEPLOY.md "Secrets" is the table of what each one is for.
Terraform's only in-cluster Secret is the operator's own vault login, which
it reads out of the vault using your `az login`. Operator-generated
...
- The Key Vault holds every password and Azure keeps it (soft-delete, 90
  days). A rebuild comes back with the same credentials.
- `terraform.tfstate` — contains Talos machine secrets and a plain-text copy
  of the vault login. Encrypt it. Never commit it.
...
- Key Vault name, tenant — `terraform.tfvars` (`azure_key_vault_name`) and
  `cluster/lab/secrets/clustersecretstore.yaml` (`vaultUrl`, `tenantId`);
  `scripts/keyvault.ps1` defaults (`-VaultName`, `-ResourceGroup`)
